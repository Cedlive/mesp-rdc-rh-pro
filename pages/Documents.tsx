...
import { FileText, Download, Upload, Search, X, Shield, Gavel, Calculator, User, LayoutGrid, ArrowUpDown, ArrowUp, ArrowDown, Trash2, Edit2, Save, CheckSquare, Square, Plus, FileSpreadsheet, File, List, Grid, History, Clock, RotateCcw, Eye } from 'lucide-react';
...
export const Documents = () => {
  ...
  const filteredDocs = useMemo(() => documents.filter(doc => (activeCategory === 'All' || doc.category === activeCategory) && (doc.name.toLowerCase().includes(searchQuery.toLowerCase()) || doc.type.toLowerCase().includes(searchQuery.toLowerCase()))).sort((a,b) => sortConfig.direction === 'asc' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name)), [activeCategory, searchQuery, documents, sortConfig]);
  ...
  const handleUploadNewVersion = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file && selectedDocument) {
          ...
          const oldVersion: DocumentVersion = {
              ...
              comment: 'Version archivée avant mise à jour'
          };

          // Calculate new version number (simple increment)
          const currentVerNum = parseFloat(selectedDocument.currentVersion);
  ...
  const handleRestoreVersion = (version: DocumentVersion) => {
      ...
      if (confirm(`Voulez-vous restaurer la version ${version.versionNumber} ? Cela créera une nouvelle version basée sur celle-ci.`)) {
          
          // Current becomes history
          const currentAsHistory: DocumentVersion = {
              id: `v_${Date.now()}_curr`,
              versionNumber: selectedDocument.currentVersion,
              updatedAt: selectedDocument.updatedAt,
              updatedBy: selectedDocument.owner,
              size: selectedDocument.size,
              comment: 'Archivé avant restauration'
          };
          ...
          const updatedDoc = {
              ...
              updatedAt: new Date().toISOString().split('T')[0],
              size: version.size,
              owner: 'Système (Restauration)',
              versions: [currentAsHistory, ...(selectedDocument.versions || [])]
          };

          setDocuments(documents.map(d => d.id === selectedDocument.id ? updatedDoc : d));
          setSelectedDocument(updatedDoc);

          window.dispatchEvent(new CustomEvent('app-alert', { detail: { title: 'Version restaurée', message: `Le document est maintenant en version ${newVerNum}.`, type: 'success' } }));
      }
  };
            ...
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100">
                ...
                <form onSubmit={handleAddDocument} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        {['salary_grid', 'contract_template', 'internal_rules', 'other'].map(t => (
                            <button type="button" key={t} onClick={() => setNewDocData({...newDocData, officialType: t, name: t==='other'?'':t})} className={`p-3 rounded-xl border text-left text-xs font-bold uppercase transition-all ${newDocData.officialType === t ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:bg-gray-50'}`}>{t.replace('_', ' ')}</button>
                        ))}
                    </div>
                    ...
                    <div className="border-2 border-dashed border-blue-200 bg-blue-50/30 rounded-xl p-8 flex flex-col items-center justify-center text-blue-400 hover:bg-blue-50 hover:border-blue-300 transition-colors cursor-pointer">
                        <Upload size={32} className="mb-2" />
                        <span className="text-sm font-medium">Glisser un fichier ici ou cliquer</span>
                        <span className="text-xs text-slate-400 mt-1">Serveur local / Poste de travail</span>
                    </div>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-50 overflow-y-auto transform transition-transform animate-in slide-in-from-right duration-300 flex flex-col">
             <div className="bg-slate-50 border-b border-slate-100 p-6 flex justify-between items-start">
                <div className="flex items-start gap-3">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${selectedDocument.type === 'PDF' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                        <FileText size={24} />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-slate-800 leading-tight">{selectedDocument.name}</h2>
                        <span className="inline-block mt-1 px-2 py-0.5 bg-white border border-slate-200 rounded text-xs font-medium text-slate-500">
                            Version actuelle: {selectedDocument.currentVersion}
                        </span>
                    </div>
                </div>
             ...
             <div className="p-6 space-y-8 flex-1 bg-white">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                     <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                         ...
                         <p className="font-semibold text-slate-700">{selectedDocument.size}</p>
                     </div>
                     <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                         <p className="text-xs text-slate-400 mb-1">Dernière modif.</p>
                         <p className="font-semibold text-slate-700">{selectedDocument.updatedAt}</p>
                     </div>
                     <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                         <p className="text-xs text-slate-400 mb-1">Propriétaire</p>
                         <p className="font-semibold text-slate-700">{selectedDocument.owner}</p>
                     </div>
                     <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                         <p className="text-xs text-slate-400 mb-1">Catégorie</p>
                         <p className="font-semibold text-slate-700">{selectedDocument.category}</p>
                     </div>
                 </div>
                 ...
                 <div>
                     <div className="flex justify-between items-center mb-4">
                         <h3 className="font-bold text-slate-800 flex items-center gap-2"><History size={18} className="text-blue-600"/> Historique des versions</h3>
                         <button 
                         ...
                         <input type="file" ref={versionFileInputRef} className="hidden" onChange={handleUploadNewVersion} />
                     </div>
                     
                     <div className="relative pl-4 border-l-2 border-slate-100 space-y-6">
                         {/* Current */}
                         <div className="relative">
                             <div className="absolute -left-[21px] top-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white ring-1 ring-green-100"></div>
                             <div className="flex justify-between items-start">
                                 <div>
                                     <p className="text-sm font-bold text-slate-800">Version {selectedDocument.currentVersion} (Actuelle)</p>
                                     <p className="text-xs text-slate-500 mt-0.5">{selectedDocument.updatedAt} • par {selectedDocument.owner}</p>
                                 </div>
                                 <span className="text-xs font-mono text-slate-400">{selectedDocument.size}</span>
                             </div>
                         </div>
                         ...
                         {selectedDocument.versions?.map((version) => (
                             <div key={version.id} className="relative group">
                                 <div className="absolute -left-[21px] top-0 w-3 h-3 bg-slate-300 rounded-full border-2 border-white group-hover:bg-blue-400 transition-colors"></div>
                                 <div className="flex justify-between items-start">
                                     <div>
                                         <p className="text-sm font-medium text-slate-600 group-hover:text-blue-600 transition-colors">Version {version.versionNumber}</p>
                                         <p className="text-xs text-slate-400 mt-0.5">{version.updatedAt} • par {version.updatedBy}</p>
                                         {version.comment && <p className="text-xs text-slate-400 italic mt-1">"{version.comment}"</p>}
                                     </div>
                                     <div className="flex flex-col items-end gap-1">
                                         <span className="text-xs font-mono text-slate-400">{version.size}</span>
                                         <button 
                                            onClick={() => handleRestoreVersion(version)}
                                            className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded flex items-center gap-1 hover:bg-blue-100"
                                         >
             ...
             <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-2">
                 <button className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2">
                     <Download size={18} /> Télécharger
                 </button>
                 {selectedDocument.status !== 'Vérifié' && (
                     <button className="flex-1 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold hover:bg-slate-50 transition-colors">
                         Valider
                     </button>
                 )}
             </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        ...
        <div className="flex gap-3">
            ...
            <div className="flex bg-white rounded-xl border border-gray-200 p-1">
                <button onClick={()=>setViewMode('list')} className={`p-2 rounded-lg ${viewMode==='list'?'bg-slate-100 text-slate-800':'text-gray-400'}`}><List size={18}/></button>
                <button onClick={()=>setViewMode('grid')} className={`p-2 rounded-lg ${viewMode==='grid'?'bg-slate-100 text-slate-800':'text-gray-400'}`}><Grid size={18}/></button>
            </div>
